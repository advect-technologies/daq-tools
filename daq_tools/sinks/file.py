...
class FileSink(AsyncSink):
    ...
    def __init__(
        ...
        self.base_dir = Path(self.sink_config.get("output_path", "output/data"))
        self.format: Literal["jsonl", "csv"] = self.sink_config.get("format", "jsonl")
        self.rotation: Literal["daily", "hourly", "weekly", "size-only"] = (
            self.sink_config.get("rotation", "size-only")
        )
        self.max_size_mb: float = float(self.sink_config.get("max_size_mb", 50.0))
        self.compress: bool = self.sink_config.get("compress", False)
        self.startup: bool = self.sink_config.get("startup", False)
    ...
    def _find_existing_active_files(self) -> List[Path]:
        ...
        candidates.sort(
            key=lambda x: (
                extract_timestamp_from_path(x).timestamp()
                if extract_timestamp_from_path(x)
                else 0
            ),
            reverse=True,
        )
    ...
    async def _ensure_file_open(self, force_new: bool = False) -> None:
        ...
        if self.format == "csv" and (
            self._csv_file_handle is None or self._csv_file_handle.closed
        ):
    ...
    async def _flush_jsonl_buffer(self) -> None:
        ...
        try:
            async with aiofiles.open(self._current_file, "a", encoding="utf-8") as dst:
                await dst.writelines(
                    (
                        DataPoint.from_json(line).to_json() + "\n"
            ...
            for bad_line in self._jsonl_buffer:
                await self._write_line_to_jail(bad_line, e, self._current_file)
            self._jsonl_buffer.clear()
    ...
    async def _append_csv_line(self, line: str) -> None:
        ...
        try:
            ...
            tag_set = {
                str(k).strip().replace(" ", "_").replace("-", "_").lower()
                for k in dp.tags
            }
            ...
            if not self._header_written:
                ...
                logger.info(
                    f"FileSink '{self.name}' wrote CSV header ({len(ordered_keys)} columns)"
                )
            else:
            ...
            logger.warning(
                f"CSV conversion failed for line in {self._current_file}: {e}"
            )
            await self._write_line_to_jail(
                line, e, self._current_file or Path("unknown")
            )
