...
async def handle_post(request: web.Request):
    ...
    try:
        ...
        print(
            f"✅ Successfully processed {len(lines) if 'lines' in locals() else len(data)} items\n"
        )

        return web.json_response(
            {
                "status": "ok",
                "received": len(data) if "data" in locals() else len(lines),
            }
        )
...
async def main():
    ...
    app.router.add_post(
        "/{path:.*}", handle_post
    )  # Accept any path (flexible for /write, etc.)
